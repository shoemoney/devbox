...
func TestTwoWayConvergeAndConflict(t *testing.T) {
	...
	baseA, _, err := Sync(A, rootA, "s", "", "", "alice", 1000, ig, guard, nil)
	if err != nil {
		t.Fatalf("A sync: %v", err)
	}

	// B clones the share into an empty dir.
	baseB, _, err := Sync(B, rootB, "s", "", "", "bob", 1001, ig, guard, nil)
	if err != nil {
	...
	baseA, _, err = Sync(A, rootA, "s", "", baseA, "alice", 1002, ig, guard, nil)
	if err != nil {
	...
	_, prB2, err := Sync(B, rootB, "s", "", baseB, "bob", 1003, ig, guard, nil)
	if err != nil {
	...
	if _, _, err = Sync(A, rootA, "s", "", baseA, "alice", 1004, ig, guard, nil); err != nil {
...
func TestPullDeleteAndEditBeatsDelete(t *testing.T) {
	...
	baseA, _, _ := Sync(A, rootA, "s", "", "", "alice", 1, ig, guard, nil)
	baseB, _, _ := Sync(B, rootB, "s", "", "", "bob", 2, ig, guard, nil)

	// A deletes gone.txt; B (clone) should drop it on next sync.
	os.Remove(filepath.Join(rootA, "gone.txt"))
	baseA, _, _ = Sync(A, rootA, "s", "", baseA, "alice", 3, ig, guard, nil)
	if _, _, err := Sync(B, rootB, "s", "", baseB, "bob", 4, ig, guard, nil); err != nil {
		t.Fatal(err)
	}
...
func TestPullSkipsFileDirClash(t *testing.T) {
	...
	pr, err := Pull(B, rootB, "s", "", "", "bob", 2, ig, guard, nil)
	if err != nil {
...
func TestSubPathMount(t *testing.T) {
	...
	baseA, _, err := Sync(A, rootA, "proj", "", "", "alice", 1, ig, guard, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	rootB := t.TempDir()
	baseB, _, err := Sync(B, rootB, "proj", "app", "", "bob", 2, ig, guard, nil)
	if err != nil {
	...
	if _, _, err = Sync(B, rootB, "proj", "app", baseB, "bob", 3, ig, guard, nil); err != nil {
		t.Fatalf("B subpath push: %v", err)
	}

	// A (whole share) gets B's edit AND still has docs/ + root.txt intact.
	if _, _, err = Sync(A, rootA, "proj", "", baseA, "alice", 4, ig, guard, nil); err != nil {
		t.Fatal(err)
	}
